...
export function SavingsCalculator() {
  ...
  useEffect(() => {
    ...
    const s = Number(q.get("strom") || 0), g = Number(q.get("gas") || 0);
    if (s > 0) setStrom(s); if (g > 0) setGas(g);
  ...
  useEffect(() => {
    ...
    fetch(`${OPENPLZ}/Localities?postalCode=${plz}&page=1&pageSize=50`, { signal: controller.signal, cache: "no-store" })
      ...
      .then((data: Locality[]) => { const names = Array.from(new Set(data.map(x => (x.name || x.municipality?.name || "").trim()).filter(Boolean))); setOrte(names); if (names.length === 1) setOrt(names[0]); })
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      fetch(`${NOMINATIM}/search?${params}`, { signal: controller.signal, cache: "no-store" }).then(r => r.ok ? r.json() : Promise.reject()).then((data: Geo[]) => setStrassen(Array.from(new Set(data.map(x => x.address?.road || "").filter(Boolean))))).catch(() => setStrassen([]));
    }, 250);
