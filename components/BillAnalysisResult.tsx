...
export function BillAnalysisResult({ analysis, onSubmitted }: Props) {
  const cards = [
    {
      key: "annualConsumptionKwh" as const,
      label: "Verbrauch",
      value: formatValue("annualConsumptionKwh", analysis.annualConsumptionKwh.value),
      large: true,
    },
    {
      key: "workPriceCtPerKwh" as const,
      label: "Arbeitspreis",
      value: formatValue("workPriceCtPerKwh", analysis.workPriceCtPerKwh.value) ?? "Nicht erkannt",
      large: true,
      alwaysVisible: true,
    },
    {
      key: "basePriceEurPerYear" as const,
      label: "Grundpreis",
      value: formatValue("basePriceEurPerYear", analysis.basePriceEurPerYear.value),
      large: false,
    },
    {
      key: "contractEnd" as const,
      label: "Vertragsende",
      value: formatValue("contractEnd", analysis.contractEnd.value),
      large: false,
    },
  ].filter((card) => card.value !== null || card.alwaysVisible);
    ...
    <div className="mt-4 overflow-hidden rounded-[30px] border border-[#1b3b58] bg-[#061a2d] shadow-2xl shadow-black/25">
      <div className="border-b border-white/10 px-5 py-4 sm:px-7">
        ...
        {analysis.provider.value && (
          <p className="mt-2 text-xs font-semibold uppercase tracking-[.18em] text-slate-500">
            {String(analysis.provider.value)}
          </p>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 gap-3 p-4 sm:grid-cols-2 sm:p-6">
        {cards.map((card) => (
          ...
          >
            ...
            <p
              className={`mt-2 font-black tracking-[-.03em] text-white ${
                card.large ? "text-3xl sm:text-4xl" : "text-xl sm:text-2xl"
              }`}
            >
              {card.value}
            </p>
      ...
      <div className="mx-4 mb-4 flex items-center gap-4 rounded-2xl border border-white/10 bg-white/[.025] p-3 sm:mx-6 sm:mb-6 sm:p-4">
        ...
        <div className="min-w-0">
          ...
          <p className="mt-1 text-xs leading-5 text-slate-400">
            Wenn du möchtest, schaue ich mir deine Tarifdaten persönlich an und erkläre dir, was du daraus machen kannst.
          </p>
