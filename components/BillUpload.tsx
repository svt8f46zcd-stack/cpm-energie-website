  ...
  const analyzeFiles = async () => {
    ...
    try {
      ...
      setError(err instanceof Error && err.message === "OCR_LIBRARY_LOAD_FAILED"
        ...
        : "Die Rechnung konnte nicht ausgelesen werden. Bitte die Seite möglichst gerade und vollständig fotografieren oder eine PDF hochladen.");
    }
  };
    ...
    <div className="flex items-start gap-3">
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          <button type="button" disabled={status === "analyzing"} onClick={() => inputRef.current?.click()} className="rounded-xl border border-[#19b7ff]/35 bg-[#19b7ff]/10 px-4 py-2.5 text-sm font-bold text-[#8ce4ff] transition hover:bg-[#19b7ff]/20 disabled:opacity-50">{files.length ? "Weitere Seite hinzufügen" : "Rechnung auswählen"}</button>
          {files.length > 0 && <button type="button" disabled={status === "analyzing"} onClick={analyzeFiles} className="rounded-xl bg-[#19b7ff] px-4 py-2.5 text-sm font-bold text-[#03101c] disabled:opacity-50">{status === "analyzing" ? "Rechnung wird geprüft …" : status === "done" ? "Erneut prüfen" : `${files.length} ${files.length === 1 ? "Datei" : "Dateien"} prüfen`}</button>}
        </div>
