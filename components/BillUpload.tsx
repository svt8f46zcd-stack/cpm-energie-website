...
export default function BillUpload() {
  ...
  const handleFile = async (file?: File) => {
    ...
    try {
      ...
      const keyValues = [
        result.energyType.value,
        result.provider.value,
        result.annualConsumptionKwh.value,
      ...
      setStatus("ready");
      setError(
        err instanceof Error && err.message === "OCR_LIBRARY_LOAD_FAILED"
        ...
        {analysis && <div className="mt-4 rounded-xl border border-white/10 bg-black/20 p-3">
          ...
          <div className="mt-3 grid gap-2 sm:grid-cols-2">
            {labels.map(([key, label]) => {
              ...
              return <div key={key} className="rounded-lg border border-white/10 bg-white/[.025] p-2.5"><p className="text-[11px] text-slate-500">{label}</p><p className="mt-0.5 text-sm font-semibold text-white">{displayValue(key, f.value)}</p><p className="mt-0.5 text-[10px] text-slate-500">Sicherheit: {f.confidence}</p></div>;
            })}
          </div>
